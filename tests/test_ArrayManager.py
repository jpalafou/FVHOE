...
def test_remove_array(array_manager):
    """Test removing an array."""
    array = np.random.rand(5, 5)
    array_manager.add("test_array", array)
    array_manager.remove("test_array")
    assert "test_array" not in array_manager.arrays


def test_remove_non_existing_array_raises_error(array_manager):
    """Test removing a non-existing array raises a KeyError."""
    with pytest.raises(KeyError):
        array_manager.remove("non_existent_array")
